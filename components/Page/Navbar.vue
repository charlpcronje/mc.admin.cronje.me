    ...
    <BuilderNavbar>
            ...
            <div class="relative hidden lg:flex items-center ml-auto">
                    ...
                    <ul class="flex items-center space-x-8">
                        <li v-for="(item, i) in menus" :key="i">
                            ...
                            <Button v-else-if="item.type === 'button'" :text="item.text" size="xs"
                                class="font-extrabold capitalize" :to="item.route ? item.route : undefined"
                                :href="item.href ? item.href : undefined" />
                        </li>
                        <li v-if="currentUser">
                            <Button text="Sign Out" size="xs" class="font-extrabold capitalize" @click="onLogoutClick" />
                        </li>
                        <li v-else="currentUser">
                            <Button text="Sign In" size="xs" class="font-extrabold capitalize" @click="onLogInClick" />
                        </li>
                    </ul>
                </nav>
                <div class="flex space-x-4 border-l ml-6 pl-6 border-gray-900/10 dark:border-gray-50/[0.2]">
                    ...
                    <Anchor
                        ...
                        href="https://github.com/charlpcronje/mc.admin.cronje.me" title="Github">
                        <IconMdi:github-face />
                    </Anchor>
                </div>
            </div>
        ...
        <template #options="{ toggleOptions }">
            <ActionSheet @on-close="toggleOptions(false)">
                    ...
                    <nav class="leading-6 font-semibold text-gray-600 dark:text-gray-300">
                        <ul class="flex flex-col">
                            <li v-for="(item, i) in menus" :key="i" class="flex w-full" :class="{
                                ...
                                <Anchor v-if="item.type === 'link'" :to="item.route ? item.route : undefined"
                                    :href="item.href ? item.href : undefined" class="flex-1 hover:no-underline capitalize">
                                    {{ item.text }}
                                </Anchor>
                                <Button v-else-if="item.type === 'button'" :text="item.text" size="xs"
                ...
                <Button type="secondary" title="Github" href="https://github.com/charlpcronje/mc.admin.cronje.me">
                    <IconMdi:github-face />
